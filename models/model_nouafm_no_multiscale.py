# Ablation: Replace multi-scale dilation branches (d=1,2,5) with single branch dilation=1 only, keep DetailGuidance
import torch
...
class Axial_PFCU_DG_Single(nn.Module):
...
class EncoderBlock(nn.Module):
    def __init__(self, in_c, out_c, mixer_kernel=(5, 5)):
        ...
        self.pfcu_dg   = Axial_PFCU_DG_Single(in_c, mixer_kernel=mixer_kernel)
...
class DecoderBlock_NoUAFM(nn.Module):
    def __init__(self, in_c, out_c, mixer_kernel=(5, 5)):
        ...
        self.pfcu_dg   = Axial_PFCU_DG_Single(gc, mixer_kernel=mixer_kernel)
...
class ULiteModel_NoUAFM_NoMultiScale(nn.Module):
...
def build_model(num_classes=1):
    return ULiteModel_NoUAFM_NoMultiScale(num_classes=num_classes)
