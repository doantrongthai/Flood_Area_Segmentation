# Ablation A9: AxialDW H/W axis decomposition replaced with isotropic square-kernel depthwise conv
import torch
...
class Axial_PFCU_DG(nn.Module):
    def __init__(self, dim, mixer_kernel=(5, 5)):
        super().__init__()
        self.branch_r1   = IsotropicDW(dim, mixer_kernel, dilation=1)
        self.branch_r2   = IsotropicDW(dim, mixer_kernel, dilation=2)
        self.branch_r5   = IsotropicDW(dim, mixer_kernel, dilation=5)
...
class BottleNeckBlock(nn.Module):
    def __init__(self, dim, max_dim=128):
        ...
        self.axial_refine = IsotropicDW(dim, mixer_kernel=(5, 5), dilation=1)
...
def build_model(num_classes=1):
    return ULiteModel_A9(num_classes=num_classes)
