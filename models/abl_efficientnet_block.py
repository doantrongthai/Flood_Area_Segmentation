...
class MBConvSE(nn.Module):
    def __init__(self, dim, mixer_kernel=(5, 5), expand_ratio=2, se_ratio=4):
        super().__init__()
        mid = dim * expand_ratio
        k   = mixer_kernel[0]
        red = max(dim // se_ratio, 4)
        self.pw_up  = nn.Conv2d(dim, mid, 1, bias=False)
        ...
        self.se_fc1 = nn.Conv2d(mid, red, 1, bias=False)
        self.se_act = nn.ReLU(inplace=True)
        self.se_fc2 = nn.Conv2d(red, mid, 1, bias=False)
        self.se_sig = nn.Sigmoid()
        self.pw_dn  = nn.Conv2d(mid, dim, 1, bias=False)
        self.bn3    = nn.BatchNorm2d(dim)

    def forward(self, x):
        out = self.act1(self.bn1(self.pw_up(x)))
        out = self.act2(self.bn2(self.dw(out)))
        se  = self.se_sig(self.se_fc2(self.se_act(self.se_fc1(self.se_avg(out)))))
        out = self.bn3(self.pw_dn(out * se))
        return out + x
...
class EncoderBlock(nn.Module):
    def __init__(self, in_c, out_c, mixer_kernel=(5, 5)):
        ...
        self.pfcu      = MBConvSE(in_c, mixer_kernel=mixer_kernel)
...
class AblModel_EfficientNet(nn.Module):
...
def build_model(num_classes=1):
    return AblModel_EfficientNet(num_classes=num_classes)
