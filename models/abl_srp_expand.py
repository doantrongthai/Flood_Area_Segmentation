...
class ExpandSRP(nn.Module):
    ...
    def forward(self, x):
        ...
        out = out + self.dw(out)
        out = self.bn_fuse(self.pw_dn(out))
        return self.act(out)
...
class EncoderBlock(nn.Module):
    def __init__(self, in_c, out_c, mixer_kernel=(5, 5)):
        ...
        self.pfcu      = ExpandSRP(in_c, mixer_kernel=mixer_kernel)
...
class BottleNeck(nn.Module):
    ...
    def forward(self, x):
        return self.act(self.bn(self.dw(x) + self.dw(x) * 0 + x))
...
class AblModel_ExpandSRP(nn.Module):
...
def build_model(num_classes=1):
    return AblModel_ExpandSRP(num_classes=num_classes)
