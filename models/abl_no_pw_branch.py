...
class EncoderBlock(nn.Module):
    def __init__(self, in_c, out_c, mixer_kernel=(5, 5)):
        ...
        if not self.same_channels:
            self.pw_expand = nn.Conv2d(in_c, out_c, kernel_size=1, bias=False)
    ...
    def forward(self, x):
        ...
        if self.same_channels:
            ...
            x = self.act(self.bn2(self.pw_expand(pool)))
        return x, skip
...
class AblModel_NoPWBranch(nn.Module):
...
def build_model(num_classes=1):
    return AblModel_NoPWBranch(num_classes=num_classes)
