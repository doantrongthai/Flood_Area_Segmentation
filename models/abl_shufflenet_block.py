...
class ShuffleBlock(nn.Module):
    def __init__(self, dim, mixer_kernel=(5, 5)):
        ...
        half = dim // 2
        k    = mixer_kernel[0]
        self.dw    = nn.Conv2d(half, half, k, padding='same', groups=half, bias=False)
        self.bn_dw = nn.BatchNorm2d(half)
        self.pw    = nn.Conv2d(half, half, 1, bias=False)
        self.bn_pw = nn.BatchNorm2d(half)
        self.act   = nn.PReLU(half)
    ...
    def forward(self, x):
        ...
        x2 = self.act(self.bn_pw(self.pw(self.bn_dw(self.dw(x2)))))
...
class EncoderBlock(nn.Module):
    def __init__(self, in_c, out_c, mixer_kernel=(5, 5)):
        ...
        self.pfcu      = ShuffleBlock(in_c, mixer_kernel=mixer_kernel)
...
class AblModel_ShuffleNet(nn.Module):
...
def build_model(num_classes=1):
    return AblModel_ShuffleNet(num_classes=num_classes)
