...
class InceptionBlock(nn.Module):
    def __init__(self, dim, mixer_kernel=(5, 5)):
        ...
        q   = dim // 4
        k   = mixer_kernel[0]
        self.b1_pw   = nn.Conv2d(dim, q, 1, bias=False)
        self.b2_pw   = nn.Conv2d(dim, q, 1, bias=False)
        self.b2_dw   = nn.Conv2d(q,   q, k, padding='same', groups=q, bias=False)
        self.b3_pw   = nn.Conv2d(dim, q, 1, bias=False)
        self.b3_dw1  = nn.Conv2d(q,   q, k, padding='same', groups=q, bias=False)
        self.b3_dw2  = nn.Conv2d(q,   q, k, padding='same', groups=q, bias=False)
        self.b4_pool = nn.AvgPool2d(3, stride=1, padding=1)
        self.b4_pw   = nn.Conv2d(dim, q, 1, bias=False)
        self.bn_out  = nn.BatchNorm2d(dim)
        self.act     = nn.PReLU(dim)

    def forward(self, x):
        b1 = self.b1_pw(x)
        b2 = self.b2_dw(self.b2_pw(x))
        b3 = self.b3_dw2(self.b3_dw1(self.b3_pw(x)))
        b4 = self.b4_pw(self.b4_pool(x))
        return self.act(self.bn_out(torch.cat([b1, b2, b3, b4], dim=1))) + x
...
class EncoderBlock(nn.Module):
    def __init__(self, in_c, out_c, mixer_kernel=(5, 5)):
        ...
        self.pfcu      = InceptionBlock(in_c, mixer_kernel=mixer_kernel)
...
class AblModel_Inception(nn.Module):
...
def build_model(num_classes=1):
    return AblModel_Inception(num_classes=num_classes)
