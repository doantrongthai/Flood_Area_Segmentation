...
class ResDWSep(nn.Module):
    def __init__(self, dim, mixer_kernel=(5, 5)):
        super().__init__()
        k = mixer_kernel[0]
        self.dw     = nn.Conv2d(dim, dim, k, padding='same', groups=dim, bias=False)
    ...
    def forward(self, x):
        out = self.act_dw(self.bn_dw(self.dw(x)))
        out = self.act_pw(self.bn_pw(self.pw(out)))
        return out + x
...
class EncoderBlock(nn.Module):
    def __init__(self, in_c, out_c, mixer_kernel=(5, 5)):
        ...
        self.pfcu      = ResDWSep(in_c, mixer_kernel=mixer_kernel)
...
class AblModel_ResDWSep(nn.Module):
...
def build_model(num_classes=1):
    return AblModel_ResDWSep(num_classes=num_classes)
