...
class FireModule(nn.Module):
    def __init__(self, dim, mixer_kernel=(5, 5)):
        ...
        ex  = dim // 2
        k   = mixer_kernel[0]
        self.squeeze    = nn.Conv2d(dim, sq, 1, bias=False)
        self.bn_sq      = nn.BatchNorm2d(sq)
        self.act_sq     = nn.PReLU(sq)
        self.expand_1x1 = nn.Conv2d(sq, ex, 1, bias=False)
        self.expand_dw  = nn.Conv2d(sq, ex, k, padding='same', groups=sq, bias=False)
        self.expand_pw  = nn.Conv2d(sq, ex, 1, bias=False)
        self.bn_ex      = nn.BatchNorm2d(dim)
        self.act_ex     = nn.PReLU(dim)

    def forward(self, x):
        sq  = self.act_sq(self.bn_sq(self.squeeze(x)))
        e1  = self.expand_1x1(sq)
        e3  = self.expand_pw(self.expand_dw(sq))
        out = self.act_ex(self.bn_ex(torch.cat([e1, e3], dim=1)))
        return out + x
...
class EncoderBlock(nn.Module):
    def __init__(self, in_c, out_c, mixer_kernel=(5, 5)):
        ...
        self.pfcu      = FireModule(in_c, mixer_kernel=mixer_kernel)
...
class AblModel_SqueezeNet(nn.Module):
...
def build_model(num_classes=1):
    return AblModel_SqueezeNet(num_classes=num_classes)
