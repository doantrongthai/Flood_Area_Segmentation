...
class GhostBlock(nn.Module):
    def __init__(self, dim, mixer_kernel=(5, 5)):
        ...
        half = dim // 2
        k    = mixer_kernel[0]
        self.primary  = nn.Conv2d(dim, half, 1, bias=False)
        self.bn1      = nn.BatchNorm2d(half)
        self.act1     = nn.PReLU(half)
        self.cheap    = nn.Conv2d(half, half, k, padding='same', groups=half, bias=False)
        self.bn2      = nn.BatchNorm2d(half)
        self.act2     = nn.PReLU(half)
        self.bn_out   = nn.BatchNorm2d(dim)

    def forward(self, x):
        p   = self.act1(self.bn1(self.primary(x)))
        g   = self.act2(self.bn2(self.cheap(p)))
        out = self.bn_out(torch.cat([p, g], dim=1))
        return out + x
...
class EncoderBlock(nn.Module):
    def __init__(self, in_c, out_c, mixer_kernel=(5, 5)):
        ...
        self.pfcu      = GhostBlock(in_c, mixer_kernel=mixer_kernel)
...
class AblModel_GhostNet(nn.Module):
...
def build_model(num_classes=1):
    return AblModel_GhostNet(num_classes=num_classes)
