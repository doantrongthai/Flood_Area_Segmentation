...
class MixConvBlock(nn.Module):
    def __init__(self, dim, mixer_kernel=(5, 5)):
        super().__init__()
        q = dim // 4
        self.dw3  = nn.Conv2d(q, q, 3, padding='same', groups=q, bias=False)
        self.dw5  = nn.Conv2d(q, q, 5, padding='same', groups=q, bias=False)
        self.dw7  = nn.Conv2d(q, q, 7, padding='same', groups=q, bias=False)
        self.dw1  = nn.Conv2d(q, q, 1, groups=1,       bias=False)
        self.pw   = nn.Conv2d(dim, dim, 1, bias=False)
        self.bn   = nn.BatchNorm2d(dim)
        self.act  = nn.PReLU(dim)

    def forward(self, x):
        splits = x.chunk(4, dim=1)
        m = torch.cat([
            self.dw3(splits[0]),
            self.dw5(splits[1]),
            self.dw7(splits[2]),
        ...
        return self.act(self.bn(self.pw(m))) + x
...
class EncoderBlock(nn.Module):
    def __init__(self, in_c, out_c, mixer_kernel=(5, 5)):
        ...
        self.pfcu      = MixConvBlock(in_c, mixer_kernel=mixer_kernel)
...
class AblModel_MixConv(nn.Module):
...
def build_model(num_classes=1):
    return AblModel_MixConv(num_classes=num_classes)
