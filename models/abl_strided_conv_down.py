...
class EncoderBlock(nn.Module):
    def __init__(self, in_c, out_c, mixer_kernel=(5, 5)):
        ...
        self.down_pool = nn.Conv2d(in_c, in_c, kernel_size=2, stride=2, groups=in_c, bias=False)
        if not self.same_channels:
            self.pw      = nn.Conv2d(in_c, conv_out, kernel_size=1, bias=False)
            self.down_pw = nn.Conv2d(conv_out, conv_out, kernel_size=2, stride=2, groups=conv_out, bias=False)
...
class AblModel_StridedConvDown(nn.Module):
...
def build_model(num_classes=1):
    return AblModel_StridedConvDown(num_classes=num_classes)
