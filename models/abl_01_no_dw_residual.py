...
class StandardDW_NoResidual(nn.Module):
...
class Axial_PFCU_Single(nn.Module):
    def __init__(self, dim, mixer_kernel=(5, 5)):
        super().__init__()
        self.branch_r1 = StandardDW_NoResidual(dim, mixer_kernel, dilation=1)
...
class SimpleBottleNeck(nn.Module):
    def __init__(self, dim, max_dim=128):
        super().__init__()
        self.dw  = StandardDW_NoResidual(dim, mixer_kernel=(5, 5), dilation=1)
...
class AblModel_NoDWResidual(nn.Module):
...
def build_model(num_classes=1):
    return AblModel_NoDWResidual(num_classes=num_classes)
