# Ablation A8: Axial_PFCU_DG replaced with standard depthwise-separable conv block (no axial, no multi-scale, no DG)
import torch
...
class EncoderBlock(nn.Module):
    def __init__(self, in_c, out_c, mixer_kernel=(5, 5)):
        ...
        self.pfcu_dg   = DSConvBlock(in_c, mixer_kernel=mixer_kernel)
...
class DecoderBlock(nn.Module):
    def __init__(self, in_c, out_c, mixer_kernel=(5, 5)):
        ...
        self.pfcu_dg = DSConvBlock(gc, mixer_kernel=mixer_kernel)
...
def build_model(num_classes=1):
    return ULiteModel_A8(num_classes=num_classes)
