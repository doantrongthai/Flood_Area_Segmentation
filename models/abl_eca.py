...
class ECA(nn.Module):
    def __init__(self, dim, mixer_kernel=(5, 5), k=3):
        super().__init__()
    ...
    def forward(self, x):
        s = self.gap(x).squeeze(-1).transpose(-1, -2)
        s = self.sig(self.conv(s)).transpose(-1, -2).unsqueeze(-1)
        return x * s
...
class EncoderBlock(nn.Module):
    def __init__(self, in_c, out_c, mixer_kernel=(5, 5)):
        ...
        self.pfcu      = ECA(in_c, mixer_kernel=mixer_kernel)
...
def build_model(num_classes=1):
    return AblModel_ECA(num_classes=num_classes)
