# Ablation A8: AxialDW (factored h×1 + 1×w) replaced with standard 3×3 depthwise conv — isolates contribution of axial decomposition for long-range spatial dependency
...
class StandardDW(nn.Module):
    def __init__(self, dim, dilation=1):
        super().__init__()
        self.dw = nn.Conv2d(dim, dim, kernel_size=3, padding=dilation, groups=dim, dilation=dilation, bias=False)
...
class Axial_PFCU_DG(nn.Module):
    def __init__(self, dim, mixer_kernel=(5, 5)):
        super().__init__()
        self.branch_r1   = StandardDW(dim, dilation=1)
        self.branch_r2   = StandardDW(dim, dilation=2)
        self.branch_r5   = StandardDW(dim, dilation=5)
...
class BottleNeckBlock(nn.Module):
    def __init__(self, dim, max_dim=128):
        ...
        self.std_refine = StandardDW(dim, dilation=1)
    ...
    def forward(self, x):
        ...
        out = self.bn_refine(self.std_refine(spp_fused))
