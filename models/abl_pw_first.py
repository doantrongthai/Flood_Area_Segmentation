...
class PWFirstSRP(nn.Module):
    ...
    def forward(self, x):
        x = self.bn_pw(self.pw_fuse(x))
        x = self.bn_fuse(self.branch_r1(x))
        return self.act(x)
...
class EncoderBlock(nn.Module):
    def __init__(self, in_c, out_c, mixer_kernel=(5, 5)):
        ...
        self.pfcu      = PWFirstSRP(in_c, mixer_kernel=mixer_kernel)
...
class AblModel_PWFirst(nn.Module):
...
def build_model(num_classes=1):
    return AblModel_PWFirst(num_classes=num_classes)
